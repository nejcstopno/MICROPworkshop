...
#-----------------------------------------------------------------------------------------
#' Exercise 3:
#' By using Brassicaceae dataset, datasets provided in the data folder or your own 
#' microbiome dataset test the performance of the code. Each group creates a folder 
#' in github repository and upload scripts, figures and short conclusions. 

#' Useful commands for git
#' Make sure to create and save the token!
#' >git clone
#' >git add
#' >git commit -m
#' >git push
...
# If using your own dataset make sure it is in right format 
# Some helpful code for data wrangling & rarefaction

library(tidyverse)
library(vegan)

# import data (from the repository folder): 
otu=read.delim('git/MICROPworkshop/<group name>/otu_table.txt', row.names = 1)
map=read.delim()

# order ASV count table and map file:
otu=otu[,order(colnames(otu))]
map=map[order(rownames(map)),]

summary(rownames(map)==colnames(otu))
